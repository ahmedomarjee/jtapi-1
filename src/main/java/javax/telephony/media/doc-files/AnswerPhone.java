...
public class 
    ...
    static String msgRecord = "Widgitek/Beep";

    /** Play this to confirm message delete. */
    static String msgDeleted = "Widgitek/Deleted";
    
    /** Record Messages to this Container. */
    static String msgBase = "Widgitek/Messages/";
	...
	String[] message = {msgSeparator, msgSeparator}; // beep + msg
	// setParameters(patterns); // a Dictionary of Pattern parameters
	setParameters(filterOn);

	Object[] msgs = cs.listObjects(msgBase, null);
	if (msgs.length == 0) return; // no messages
	// else sort by message number:
	Arrays.sort(msgs, mcomp);

	for (int i = 0; i<msgs.length; i++) {
	    message[1] = msgBase+(String)msgs[i];
     ...
     * @return a String MDO name in the base container.
     */
    String newMsgName(String base) throws ContainerException {
	String msgid = "msg-1";
	Object[] msgs = cs.listObjects(base, null);
	if (msgs.length > 0) {
	    // find largest message number and increment:
	    Arrays.sort(msgs, mcomp);
	    String mx = (String)msgs[msgs.length-1];
	    int x = mx.indexOf("-")+1;
    ...
    static final Comparator mcomp = new Comparator() {
		...
		int x1 = ((String)m1).indexOf("-")+1;
		int x2 = ((String)m2).indexOf("-")+1;
		// Integer.compareTo(...)
		return (Integer.parseInt(((String)m1).substring(x1))
			-
			Integer.parseInt(((String)m2).substring(x2))
			);
	    }
	};
}
