...
public class FollowMe extends BasicMediaService implements Runnable {
    ...
    CallControlCall myCall;
	 ...
	 String subscriberID = validateSubscriber();
	...
	String subscriberID = myCall.getCalledAddress().getName();
	String callerID = getCallerID(); // let caller express themselves
	try {
	...
	String input = promptForInput("Press 1 to Leave Message, 2 to Wait",1);
	if (input.equals("2"))
	    return true;
	else
	    return false;
     ...
     */
    String promptForInput(String prompt, int inputLength) throws Exception {
	while (true) {
	    // play the prompt
     ...
     *
     * @param subscriberID a String representation of the subscribers Address
     * @param message a String to be displayed by the page.
     */
    void sendpage(String subscriberID, String message) {
	// TBD
    }
     ...
     */
    String validateSubscriber() throws Exception {
	String subscriberID = promptForInput("Enter subscriberID", 10);
	String passwd = promptForInput("Enter your password", 4);
    ...
    String getCallerID() throws Exception {
